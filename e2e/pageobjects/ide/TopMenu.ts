...
export class TopMenu {
    ...
    public async waitTopMenu(timeout = TestConstants.TS_SELENIUM_DEFAULT_TIMEOUT) {
    ...
    public async clickOnTopMenuButton(buttonText: string, timeout = TestConstants.TS_SELENIUM_DEFAULT_TIMEOUT) {
        const buttonLocator: By = this.getTopMenuButtonLocator(buttonText);
        await this.driverHelper.waitAndClick(buttonLocator, timeout);
    }

    public async clickOnSubmenuItem(itemText: string, timeout = TestConstants.TS_SELENIUM_DEFAULT_TIMEOUT) {
